...
for _, v in pairs(playerGui:GetChildren()) do
    if v.Name == "MobileProMax_V24" then v:Destroy() end
end
pcall(function()
...
gui.DisplayOrder = 100000 -- Ép nổi lên trên cùng (Đè chữ DỪNG, Vòng...)
gui.IgnoreGuiInset = true -- Bỏ qua khoảng trống thanh bar của Roblox
pcall(function() gui.Parent = game:GetService("CoreGui") end)
if gui.Parent == nil then gui.Parent = playerGui end
...
frame.Position = UDim2.new(0.5, -170, 1.5, 0) -- Giấu tít xuống dưới màn hình lúc đầu
...
local tab2, ind2 = createTab("🌍THẾ GIỚI", 0.2, 0.2)
local tab3, ind3 = createTab("💻TIỆN ÍCH", 0.4, 0.2)
local tab4, ind4 = createTab("📍SAVE V1", 0.6, 0.2)
...
local function createPage()
    ...
    pg.CanvasSize = UDim2.new(0, 0, 0, 750); pg.ScrollBarThickness = 2
...
openBtn.MouseButton1Click:Connect(function()
    ...
    frame:TweenPosition(opened and UDim2.new(0.5, -170, 0.5, -210) or UDim2.new(0.5, -170, 1.2, 0), "Out", "Back", 0.4)
end)
...
-- [TAB 5: TP PLAYER]
local refreshBtn = createButton(page5, "🔄 LÀM MỚI DANH SÁCH", Theme.Brand, function() end)

local function loadPlayersList()
    for _, child in pairs(page5:GetChildren()) do
        if child:IsA("Frame") and child.Name == "PlayerItemFrame" then
            child:Destroy()
        end
    end
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= player then 
            ...
            item.Name = "PlayerItemFrame"
            item.Size = UDim2.new(0.92, 0, 0, 48)
            ...
            tpBtn.Position = UDim2.new(0.71, 0, 0.175, 0)
            tpBtn.Text = "ĐẾN"
            tpBtn.BackgroundColor3 = Theme.AccentOn
...
refreshBtn.MouseButton1Click:Connect(loadPlayersList)
loadPlayersList()
